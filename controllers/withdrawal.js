...
async function withdrawalWithFlutterwave(req, res, next) {
    try {
        ...
        const flw = new Flutterwave('FLWPUBK_TEST-b99b789f82767040919cb317c30417d8-X','FLWSECK_TEST-eb571a6733c62d08e25770331f287a6d-X')
...
async function withdrawalWithPaystack(req, res, next) {
    try {
...
async function storeBankDetails(req, res,next){
    try{
      const {bankName, accountNumber, accountName, bankCode,userId, updateData} = req.body

      console.log(bankName,accountNumber, accountName, bankCode, userId, updateData)

      const findUserBank = await Banks.find({userId})

      if(!findUserBank || findUserBank.length === 0) {
        ...
        return res.json({message: newBank})
      }
      else {
        const updateBankDetails = await Banks.findByIdAndUpdate(findUserBank[0]._id, updateData,{new: true})

        return res.json({message: updateBankDetails})
      }
    }
    catch(err){
        console.log(err)
...
async function getBankDetails(req, res,next){
    try{
      ...
      console.log(findUserBank[0], 'dd')

      if(!findUserBank || findUserBank.length === 0) return next(customError('Bank details not found'))

        

        return res.json({message: findUserBank[0]})
